...
export async function GET(
  ...
  try {
    ...
    const response = {
      ...
      position: employee?.position?.title || review.position || '',
      email: employee?.email || '',
      datehired: employee?.datehired?.date || '',
      department: review.department,
      ForRegular: review.ForRegular || 'Not Set',
      status: review.status,
      lastModified: review.submittedAt || review.updatedAt || review.createdAt,
      immediateSupervisorInput: review.immediateSupervisor || '',
      performanceCoverage: review.performanceCoverage || '',
      scores: scores,
